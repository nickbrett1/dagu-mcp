...
# "dagu-mcp-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "dagu-mcp-dev" is overwritten by
...
PROJECT_NAME="dagu-mcp"
AGENT_NAME="dagu-mcp-dev"
WORKSPACE="/workspaces/dagu-mcp"
...
if [ -x "/workspaces/dagu-mcp/scripts/agent-dev.sh" ]; then
  "/workspaces/dagu-mcp/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
