...
`ghcr.io/nickbrett1/dagu-mcp` on every push to
...
(`ghcr.io/nickbrett1/dagu-mcp:buildcache`) and
...
- **Package visibility** defaults to `public` (so the NAS/Watchtower can pull
  ...
  `org.opencontainers.image.source=https://github.com/nickbrett1/dagu-mcp`,
...
2. In `docker-compose.yml`, change the image to the pinned SHA:
   `image: ghcr.io/nickbrett1/dagu-mcp:<commit-sha>`
